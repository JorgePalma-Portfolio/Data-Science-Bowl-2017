...
for(dicomImage in c(1,2,4:7)){
  ...
  write.table(submissionResult,paste0("output/Submission/",dicomImage,"-",submissionfiles[dicomImage]$id,".csv"),row.names = F,col.names=F,sep=",")
  gc()
}

########################################################################################

candidates <- data.matrix(patchdata)
