...
candidates <-read.csv("input/LUNA/candidates.csv",header = T, stringsAsFactors = F)

outputFile <- "output/LUNA/LUNA_candidates.csv"
outfile <- file(outputFile, open="w+")

seriesuid <- "" 

for( imageline in 1:nrow(candidates)) {
  
  if (candidates$seriesuid[imageline] != seriesuid) {
    
    seriesuid <- candidates$seriesuid[imageline]
    
    rawfile <- paste0(candidates$seriesuid[imageline],".raw")
    mhdfile <- paste0(candidates$seriesuid[imageline],".mhd")
    ...
    worldCoord <- data.matrix(candidates[imageline,c(2,3,4)])
  ...
  ImagePatch <- normalizePlanes(newImage[,,z])
  
  if ((x1 >= 1) & (y1 >=1)  & (x2 <= new_shape[1])  & (y2 <= new_shape[2])) {
    
    if (candidates$class[imageline] == 1){
      ...
      patch1 <- ImagePatch[x1:x2, y1:y2]
      outputLine1 <- paste(c("0",as.vector(patch1)), collapse=",")
      writeLines(outputLine1,outfile)
    }
  }
}
