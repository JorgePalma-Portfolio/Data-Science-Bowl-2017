...
path <- "D:/R Development/Data Science Bowl 2017/input/VIA/"
annotations <-read.csv(paste0(path,"annotations.csv"),header = T, stringsAsFactors = F)

outputFile <- "output/VIA/VIA_annotations.csv"
...
for( imageline in 1:nrow(annotations)) {
  ...
  if (annotations$FILE[imageline] != seriesuid) {
    ...
    filepath <- "D:/R Development/Data Science Bowl 2017/input/VIA/files/"
  ...
  z  <- round(voxelCoord[3] * real_resize_factor[3],0)
  
  ImagePatch <- newImage3[,,z]
