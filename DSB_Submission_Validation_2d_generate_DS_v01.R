...
prefix <- '20170319xLUNAmodel-ver02-25-step2'
iteration <- 25
  ...
  for(sliceNumber in 1:dim(newImage)[3]){
    ...
    if (nrow(ImagePatches) > 0){
      ...
      for(i in 1:nrow(ImagePatches)){
        ...
        patchList[[patchListcnt]] <- searchPatches(img,patchsize,stride)
        patchListcnt <- patchListcnt + 1
      }
      ...
      #   tpreds      <- t(preds)
      #   pred.label  <- ifelse(tpreds[,2] >= 0.96, 1,0)
