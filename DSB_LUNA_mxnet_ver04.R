...
# first conv
conv1 <- mx.symbol.Convolution(data=data, kernel=c(7,7), num_filter=10)
tanh1 <- mx.symbol.Activation(data=conv1, act_type="tanh")
pool1 <- mx.symbol.Pooling(data=tanh1, pool_type="max", kernel=c(2,2), stride=c(1,1))

# second conv
conv2 <- mx.symbol.Convolution(data=pool1, kernel=c(7,7), num_filter=20)
...
#Dropuot
dropout <- mx.symbol.Dropout(flatten)
fc1 <- mx.symbol.FullyConnected(data=dropout, num_hidden=20)
...
model <- mx.model.FeedForward.create(lenet, X=train.array, y=train.y,
                                     ...
                                     learning.rate=0.07, momentum=0.9, wd=0.00001,
