...
for(i in 1:10){
  model[[i]] <- xgb.load(paste0("xgboost.model_v1_",i))
}
...
for(dicomImage in c(1:198)){
  ...
  for(sliceNumber in 1:dim(newImage)[3]){
    ...
    if (nrow(ImagePatches) > 0){
      ...
      if (dim(patchArray)[3] >=1){
        ...
        dim(test.array) <- c(1681, dim(patchArray)[3])
        
        test.array <- t(test.array)
        
        xgsub <- xgb.DMatrix(test.array)
        predicted.s <- list()
        ...
        pred.label <- ifelse(p.ensemble >= 0.35, 1,0)
        ...
        if (spred.label > 0){
          ...
          candidatesProbs <- p.ensemble[pred.label.log] 
  ...
  if (!(is.data.frame(patchdata) & nrow(patchdata)==0)){
     ...
     mprobs    <- matrix(unlist(probabilities),ncol=1,byrow=T)
     submissionResult <- cbind(mprobs,patchdata)
  } else {
    
    submissionResult <- t(rep(0,1683))
  }
  submissionResult <- submissionResult[which.max(submissionResult$mprobs),]
  write.table(submissionResult,paste0("output/SubmissionXGB/",submissionfiles[dicomImage]$id,".csv"),row.names = F,col.names=F,sep=",")
  rm(mprobs,submissionResult)
  gc()
}