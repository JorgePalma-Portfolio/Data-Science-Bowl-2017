...
prefix <- '20170313xLUNAmodel-ver02-25-step0'
iteration <- 25
...
#for(dicomImage in c(1:4,6,9,11:13,15:18)){
for(dicomImage in c(13,15:18)){
  
  dicom <- readDICOM(path = paste0("input/images/",Result[dicomImage]$id), verbose=TRUE)
  ...
  for(sliceNumber in 1:dim(newImage)[3]){
    ...
    if (nrow(ImagePatches) > 0){
      ...
      if (dim(patchArray)[3] >=1){
        ...
        tpreds <- t(preds)
        pred.label <- ifelse(tpreds[,2] >= 0.50, 1,0)
        ...
        if (!is.na(zdim)) {
          ...
          patchdata <- rbind(patchdata,data.frame(class=rep(0,dim(candidates)[1]),candidates))
          #tp <- t(preds)
          #colnames(probabilities) <- c("V1","V2")
          #probabilities <- rbind(probabilities,tp[pred.label.log,])
        }
      }
    }
  }
  ...
  write.table(patchdata,paste0("output/KAGGLE/Newxclass0-v",dicomImage,".csv"),row.names = F,col.names=F,sep=",")
  gc()
}
